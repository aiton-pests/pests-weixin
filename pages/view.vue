...
export default {
  ...
  methods: {
    ...
    async fetchDataById (id, type) {
      const pest = await this.$axios.$get(process.env.baseUrl + `/educenter/pests/getPestsControl/${id}`)
      if (pest.success) {
        if (pest.data.teacher != null) {
          this.pests = pest.data.teacher
          if (type === 'fellPic') {
            this.picurl = pest.data.teacher.fellPic
          } else if (type === 'finishPic') {
            this.picurl = pest.data.teacher.finishPic
          // eslint-disable-next-line no-empty
          } else if (type === 'stumpPic') {
            this.picurl = pest.data.teacher.stumpPic
          }
          // eslint-disable-next-line no-console
